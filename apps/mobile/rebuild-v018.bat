...
set "LOG=%MOBILE%\rebuild-v018.log"

echo === v0.1.8 rebuild started %DATE% %TIME% === > "%LOG%"
...
copy /Y "%MOBILE%\src-tauri\gen\android\app\build\outputs\apk\universal\release\app-universal-release.apk" "%ROOT%\releases\hacash-wallet-mobile-v0.1.8-arm64.apk" >> "%LOG%" 2>&1

echo === v0.1.8 rebuild OK %DATE% %TIME% === >> "%LOG%"
exit /b 0

:fail
echo === v0.1.8 rebuild FAILED %DATE% %TIME% === >> "%LOG%"
exit /b 1