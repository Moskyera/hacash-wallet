  ...
  return (
    <div className="balance-portfolio">
      ...
      {!hideBalances && portfolio && hacdCount != null && hacdCount > 0 ? (
        <p className="muted small balance-usd-note">{HACD_MARKET_REFERENCE_NOTICE}</p>
      ) : null}
      {!hideBalances && priceStatus === "unavailable" && (
        <p className="muted small balance-usd-note">{t("prices.unavailable")}</p>
      )}
      {!hideBalances && priceStatus === "stale" && (
        <p className="muted small balance-usd-note">{t("prices.stale")}</p>
      )}
    </div>
  );
}