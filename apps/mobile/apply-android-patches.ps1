...
# Sync Android versionCode/versionName so upgrades replace broken installs (was stuck at 1000).
$pkgJsonPath = Join-Path $mobile "package.json"
$tauriPropsPath = Join-Path $android "app\tauri.properties"
if ((Test-Path $pkgJsonPath) -and (Test-Path $tauriPropsPath)) {
...
"@ | Set-Content -Path $tauriPropsPath -Encoding UTF8 -NoNewline
        Write-Host "Set Android versionName=$ver versionCode=$versionCode" -ForegroundColor Green
    }
}
