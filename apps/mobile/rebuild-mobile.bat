...
for /f "delims=" %%V in ('powershell -NoProfile -Command "(Get-Content '%MOBILE%\package.json' -Raw | ConvertFrom-Json).version"') do set "VER=%%V"
set "LOG=%MOBILE%\rebuild-mobile.log"
...
echo APK: %ROOT%\releases\hacash-wallet-mobile-v%VER%-arm64.apk
exit /b 0
