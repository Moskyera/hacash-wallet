...
call yarn tauri build --features agent-wallet-testnet-pilot >> "%LOG%" 2>&1
if errorlevel 1 goto fail

if not exist "%ROOT%\releases" mkdir "%ROOT%\releases"

copy /Y "%ROOT%\target\release\bundle\nsis\HPAY Wallet_%VER%_x64-setup.exe" "%ROOT%\releases\hacash-wallet-desktop-v%VER%-x64-setup.exe" >> "%LOG%" 2>&1
copy /Y "%ROOT%\target\release\bundle\msi\HPAY Wallet_%VER%_x64_en-US.msi" "%ROOT%\releases\hacash-wallet-desktop-v%VER%-x64.msi" >> "%LOG%" 2>&1
