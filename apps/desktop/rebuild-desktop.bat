...
copy /Y "%ROOT%\target\release\bundle\nsis\Hacash Wallet_0.1.0_x64-setup.exe" "%ROOT%\releases\hacash-wallet-desktop-v%VER%-x64-setup.exe" >> "%LOG%" 2>&1
copy /Y "%ROOT%\target\release\bundle\msi\Hacash Wallet_0.1.0_x64_en-US.msi" "%ROOT%\releases\hacash-wallet-desktop-v%VER%-x64.msi" >> "%LOG%" 2>&1
