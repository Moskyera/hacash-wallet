/**
 * A fake Tauri IPC layer, for looking at the desktop UI in a plain browser.
 *
 * WHY THIS EXISTS
 * The desktop UI only renders real content once the Rust side answers. Building
 * Tauri to review a card's spacing costs minutes per iteration, so this answers
 * the same commands with fixed data and lets the real components, the real CSS
 * and the real layout render in a browser tab.
 *
 * WHAT IT IS NOT
 * It is not a wallet. Every number here is invented, nothing is signed, nothing
 * reaches a node, and no key exists. It refuses to install itself outside a dev
 * server so it cannot reach a packaged build. `preview.html` is also absent from
 * the production Rollup input, so it is not bundled in the first place.
 */
...
/**
 * Figures chosen to sit near the design mockups so the two can be compared side
 * by side, while staying internally consistent: every USD value below is what
 * the wallet's own computePortfolioUsd would derive from these balances and
 * these prices, rather than a number typed in to look good.
 */
...
const RESPONSES: Record<string, unknown> = {
  ...
  wallet_platform_security_status: {
    native_biometric_available: false,
    platform: "windows",
  },
...
/**
 * Anything the screens ask for that is not listed above.
 *
 * Returns an empty value rather than throwing, so one unlisted command cannot
 * blank the screen being reviewed. Shapes are guessed from the command name.
 */
function fallback(command: string): unknown {
  // Tauri's own plugin surface. get_all_* is iterated by the caller, so an
  // empty list is the only safe empty answer; null crashes the webview module.
  if (/^plugin:\w+\|get_all_/.test(command)) return [];
  if (command.startsWith("plugin:event|")) return 0;
  if (/(_list|_history|_bills|_inbox|summaries|discover)/.test(command)) return [];
  return null;
}
...
export function installPreviewIpc(): void {
  ...
  globalWindow.__TAURI_INTERNALS__ = {
    invoke: async (command: string, args: InvokeArgs) => {
      ...
      // Kept visible: the point of the harness is knowing which screen data is
      // fixture and which command has no fixture behind it yet.
      console.info(`[preview ipc] ${command}${known ? "" : " (no fixture, empty reply)"}`, args ?? {});
  ...
  // The event plugin reaches for this by name when a listener is torn down.
  // Without it every unmount throws and React unmounts the tree it was in.
...
/**
 * Seeds the price cache the shared UI reads on mount, so the portfolio card has
 * a price on the very first paint instead of after a round trip.
 */
export function seedPreviewPriceCache(): void {
  ...
  // Open straight into My Wallet rather than the space-choice screen.
  window.localStorage.setItem("hpay.wallet-space.v1", "personal");
}
