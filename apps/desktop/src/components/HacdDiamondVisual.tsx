...
type Props = {
  name: string;
  /** Kept for callers; full poster always uses the Explorer layout. */
  size?: "sm" | "lg";
};
...
/**
 * Single Explorer-style HACD metadata card (all-in-one poster).
 * Same layout as Harbor / explorer.hacash.org/diamond/{NAME} — not a bare HIP-5 chip.
 */
export default function HacdDiamondVisual({ name }: Props) {
  ...
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      setScale(Math.max(0.12, Math.min(0.55, w / 800)));
    };
    fit();
...
function ExplorerMetadataCard({
  ...
  const openExplorer = async (e: React.MouseEvent) => {
    e.preventDefault();
    try {
      await open(`${EXPLORER_BASE}${encodeURIComponent(displayName)}`);
    } catch {
      /* ignore */
    }
  };
  ...
  return (
    <article className="hacd-metadata-card" data-hacd-name={displayName}>
      ...
      {info.metadata_source === "mainnet" && (
        <p className="muted small-note hacd-meta-source">Mainnet metadata (read-only)</p>
      )}
    </article>
  );
}
