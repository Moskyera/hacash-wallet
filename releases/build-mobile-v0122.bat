...
set "LOG=%ROOT%\releases\mobile-build.log"
set "VER=0.1.22"

echo === v%VER% mobile build started %DATE% %TIME% === > "%LOG%"
...
echo === v%VER% mobile build OK %DATE% %TIME% === >> "%LOG%"
exit /b 0

:fail
echo === v%VER% mobile build FAILED %DATE% %TIME% === >> "%LOG%"
exit /b 1