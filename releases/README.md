...
**File:** `hacash-wallet-mobile-v0.1.2-arm64.apk`  
**Target:** Android arm64 (most phones, including GrapheneOS)  
**Signed:** yes (release keystore)  
**Backup:** disabled (`allowBackup=false`)  
**Fixes in v0.1.2:**
- Branded Hacash launcher icon (black adaptive background, golden mark from `hb-icon.png`)
- Vault writes to app-internal storage (fixes "read only file system error 30", carried from v0.1.1)

### Direct download (GitHub)

```
https://github.com/Moskyera/hacash-wallet/releases/download/v0.1.2-mobile/hacash-wallet-mobile-v0.1.2-arm64.apk
```
...
```powershell
Get-FileHash releases\hacash-wallet-mobile-v0.1.2-arm64.apk -Algorithm SHA256
```
