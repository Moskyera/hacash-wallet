...
```text
node scripts/release-version.mjs set 0.1.55
node scripts/release-version.mjs check 0.1.55
```
...
Linux release contains deb, AppImage, and a raw `-x64-binary`. AppImage is the
...
in v0.1.55 and must not be advertised as built or tested artifacts.
